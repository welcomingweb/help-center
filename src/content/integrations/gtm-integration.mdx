...
description: "Install the Welcoming Web Accessibility Widget via the GTM Community Template Gallery (recommended) or from file."
---
...
In **GTM → Templates → Tag Templates → Search Gallery**, search for **Welcoming Web Accessibility Widget** and click **Add to workspace**.

<GifPreview
  src="/gifs/gtm-gallery-add.webp"
  caption="Adding the Welcoming Web template from the Community Template Gallery"
/>

### Step 2: Create a tag

Go to **Tags → New → Welcoming Web Accessibility Widget**, name your tag, and fill out:

- **Widget ID (UUID)** — paste your widget ID from the Welcoming Web dashboard.

<GifPreview
  src="/gifs/gtm-create-tag.webp"
  caption="Creating the Welcoming Web tag and entering the Widget ID"
/>
...
<GifPreview
  src="/gifs/gtm-import-template.webp"
...
All styling, positioning, and behavior defaults are controlled by your server-side configuration (Welcoming Web dashboard).
...
- **Duplicate widget?** Make sure only one Welcoming Web tag fires per page view.
